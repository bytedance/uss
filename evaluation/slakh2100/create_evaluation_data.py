    ...
    while True:
        ...
        for sub_dir in sub_dirs:
            ...
            for key in stems_dict.keys():
                ...
                if plugin_name not in count_dict.keys():
                    count_dict[plugin_name] = 0

                if count_dict[plugin_name] < eval_segments_per_class:

                    source, origin_sr = librosa.load(path=source_path, sr=None, mono=True)
                    mixture, _ = librosa.load(path=mixture_path, sr=None, mono=True)
                    ...
                    # if len(candidate_indexes) > 0:
                    #     index = random_state.choice(candidate_indexes)

                    begin_second = index * segment_seconds
                    ...
                    meta_dict["source1_name"].append(sub_dir.name)
