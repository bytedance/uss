...
class SedMix(object):
    ...
    def get_mix_data5b(self, data_dict): 
    ...
    def get_eval(self, data_dict, data_type): 
        ...
        for i in range(0, len(indexes), 2):
            ...
            if False:
                ...
                data_dict['target'][K]
                [config.ix_to_lb[idx] for idx in np.argsort(seg_predictions[K])[::-1][0:10]]
                librosa.output.write_wav('_zz.wav', data_dict['waveform'][K], sr=16000)
                librosa.output.write_wav('_zz2.wav', seg_waveforms[K], sr=16000)
                seg_predictions[K, data_dict['class_id'][K]]
                # seg_predictions[K]
                # np.max(framewise_output[K, :, data_dict['class_id'][K]])
            ...
            if data_type == 'mixture':
                ...
                # Mixutres
                mixtures.append(seg_waveforms[n1])
                mixtures.append(seg_waveforms[n2])
                
                # Targets
                sources.append(seg_waveforms[n1])
                sources.append(seg_waveforms[n2])

            elif data_type == 'silence':
                # Mixutres
                mixtures.append(seg_waveforms[n2])
                mixtures.append(seg_waveforms[n1])
                
                # Targets
                sources.append(np.zeros_like(seg_waveforms[n1]))
                sources.append(np.zeros_like(seg_waveforms[n2]))
...
class SedMix_16k(object):
    def __init__(self, sed_model, at_model, segment_frames):
        self.sed_model = sed_model
        self.at_model = at_model
        self.segment_frames = segment_frames
        self.hop_samples = config.hop_samples
    ...
    def get_mix_data4b(self, data_dict): 
        ...
        for n in range(0, audios_num, 2):
            ...
            if False:
                ...
                data_dict['target'][K]
                [config.ix_to_lb[idx] for idx in np.argsort(seg_predictions[K])[::-1][0:10]]
                librosa.output.write_wav('_zz.wav', data_dict['waveform'][K][0 :: 2], sr=16000)
                librosa.output.write_wav('_zz2.wav', seg_waveforms[K], sr=16000)
                seg_predictions[K, data_dict['class_id'][K]]
                np.max(framewise_output[K, :, data_dict['class_id'][K]])
    ...
    def get_mix_data5(self, data_dict): 
        ...
        for n in range(audios_num):
            ...
            seg_waveforms.append(data_dict['waveform'][n, bgn_sample : end_sample : 2])
    ...
    '''
    def get_eval_mixture(self, data_dict, data_type): 
        ...
        for n in range(0, audios_num, 2):
            ...
            if data_type == 'mixture':
                ...
                # Mixture
                mixtures.append(seg_waveforms[n])
                mixtures.append(seg_waveforms[n + 1])
                ...
                sources.append(np.zeros_like(seg_waveforms[n]))
                sources.append(np.zeros_like(seg_waveforms[n + 1]))
