...
# Train
CUDA_VISIBLE_DEVICES=3 python3 pytorch/ss_main.py train --workspace=$WORKSPACE --at_checkpoint_path=$AT_CHECKPOINT_PATH --data_type='balanced_train' --model_type='UNet' --loss_type='mae' --balanced='balanced' --augmentation='none' --mix_type='5' --batch_size=12 --learning_rate=1e-3 --resume_iteration=0 --early_stop=1000001 --cuda
...
CUDA_VISIBLE_DEVICES=3 python3 pytorch/ss_main.py inference_new --workspace=$WORKSPACE --at_checkpoint_path=$AT_CHECKPOINT_PATH --data_type='balanced_train' --model_type='UNet' --loss_type='mae' --balanced='balanced' --augmentation='none' --batch_size=2 --iteration=100000 --cuda
